...
- Production infrastructure is defined in `infra/musicwire-production.yaml`; `scripts/deploy-production.sh` builds, publishes, and deploys the on-demand ECS Fargate service through the shared API Gateway VPC Link and internal NLB. It reads CDP credentials from an uncommitted environment file and provisions them only as ECS Secrets Manager references. The deployment uses task-local SQLite and artifact storage, so durable job recovery across task replacement is an outstanding production gap.
