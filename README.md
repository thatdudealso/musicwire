...
`X402_RPC_URL` defaults to the RPC endpoint of the configured network. Two checks keep it honest: `src/config.js` refuses a URL whose hostname identifiably names a different network, and `src/server.js` calls `eth_chainId` on the configured endpoint before binding a port and refuses to start production unless the chain it serves matches `X402_NETWORK`. That second check is fail-closed - an unreachable or wrong-chain RPC endpoint stops the boot rather than silently stranding settlements in `settlement_pending`.

#### Data retention

The 30-day lifecycle rule applies only to rendered artifact objects in S3. Nothing prunes the SQLite database: `jobs`, `payment_wallets`, `payment_authorizations`, and `validate_results` rows are retained indefinitely on EFS, so a payment, settlement, or receipt record always outlives the artifact it paid for and artifact expiry can never destroy payment proof. `expireIdempotencyKeys()` only ages out replay-protection keys within their window; it deletes no payment or job record. Any future data-retention or privacy policy that would delete these records is a captain decision, not a maintenance change.
