...
Render idempotency keys replay for 24 hours only for the same verified payer and request context; validation keys retain the same payer and MusicXML context indefinitely. Artifact retention is a 30-day minimum without an automated purge job, so deployments must provision storage and add cleanup policy before long-term operation.
