...
QC passes only when MusicXML validates, MuseScore exits successfully, every requested artifact exists, requested audio has a valid container, non-silent RMS, and score-duration agreement within 10%. MP3 exports receive a bounded three-second renderer release-tail allowance; declared sustained instruments receive up to 3.25 seconds. Musicwire derives the larger allowance from a declared `part-name` or `instrument-name` such as violin, cello, wind, brass, organ, or voice. Optional key, tempo, and duration constraints must also match. Failures return a typed catalogued error and are not charged.
