...
export class PaymentService {
  async createPendingCharge({ jobId, priceUsd }) {
    return { provider: 'stub', status: 'pending_qc', job_id: jobId, amount_usd: priceUsd, captured_at: null };
  }

  async captureAfterQc(payment) {
    if (payment.status !== 'pending_qc') throw new Error('Payment capture is only permitted after pending QC.');
